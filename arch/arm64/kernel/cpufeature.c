			 ...
			 struct cpuinfo_arm64 *boot)
{
	 ...
	 */
	if (part_num != ARM_CPU_PART_NEOVERSE_V1) {
		taint |= check_update_ftr_reg(SYS_ID_AA64MMFR0_EL1, cpu,
					      info->reg_id_aa64mmfr0, boot->reg_id_aa64mmfr0);
		taint |= check_update_ftr_reg(SYS_ID_AA64MMFR1_EL1, cpu,
					      info->reg_id_aa64mmfr1, boot->reg_id_aa64mmfr1);
	}
	if (part_num != ARM_CPU_PART_CORTEX_A77)
	...
	if (id_aa64pfr0_32bit_el0(read_sanitised_ftr_reg(SYS_ID_AA64PFR0_EL1)) &&
		...
		if ((part_num != ARM_CPU_PART_NEOVERSE_V1) && (part_num != ARM_CPU_PART_CORTEX_A77))
...
verify_local_elf_hwcaps(const struct arm64_cpu_capabilities *caps)
{
	...
	for (; caps->matches; caps++)
		if (cpus_have_elf_hwcap(caps) && !caps->matches(caps, SCOPE_LOCAL_CPU)) {
			if ((part_num == ARM_CPU_PART_NEOVERSE_V1) && (caps->sys_reg == SYS_ID_ISAR5_EL1))
				continue;
